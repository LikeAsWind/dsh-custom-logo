...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * to a fetch. The mtime is appended as `?v=` for cache busting.
     */
     ...
     * returns null" behavior: once a slot has an occupant, that occupant's
     * output (even null) replaces the shell fallback. So the occupants must be
     * registered conditionally, decided synchronously from the state the host
     * half injected into index.html, otherwise an unconfigured install blanks
     * the whole brand row.
     */
    function apply(ctx) {
