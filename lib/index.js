...
function tapIndexHtml(html) {
  ...
  const css = [];
  if (state.wide !== null) {
    // Expanded sidebar: hide shipped wordmark, paint wide logo on the brand
    // button (locked to 42px height; the logoRow content area is 44px). The
    // min-height is required — hiding the wordmark SVG would otherwise
    // collapse the button to 0px.
    css.push(
