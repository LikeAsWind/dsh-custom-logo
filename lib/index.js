...
function tapIndexHtml(html) {
  ...
  if (state.square !== null) {
    ...
    css.push(
      ...
      // Empty-conversation hero ("探索未至之境"): the whale mark is rendered
      // via slot conversation.hero.brand.mark with no data-slot wrapper, so
      // target its CSS-module hitbox. Hide the inner fish svg (already caught
      // by the viewBox rule above, but pin it) and paint the square logo into
      // the hitbox, sized to the 34x25 whale it replaces.
      '[class*="_fishHitbox"] svg{display:none!important}',
      `[class*="_fishHitbox"]{width:34px!important;height:25px!important;background:url(/custom-logo/logo-64.png?v=${state.square}) no-repeat center/contain!important}`
    );
  }
