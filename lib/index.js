...
/** Pure index.html transform — rewrites the tab favicon and injects the sync state. */
function tapIndexHtml(html) {
  ...
  // occupies a brand slot when the matching logo is present — otherwise it
  // must leave the slot empty so the shell's default brand renders.
